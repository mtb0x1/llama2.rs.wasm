...
impl Llama2GgufLoader {
  pub fn new(path: &str) -> Result<Self> {
    let inner = GGUFFileLoader::new(path)
      .map_err(|err| Llama2Error {
        kind: Llama2ErrorKind::IOError,
  ...
  fn load_weights<'a>(
    ...
    let token_embedding_table = {
      let (tensor, dims) = Self::load_tensor(gf, "token_embd.weight").unwrap();
      let tensor = tensor.view(&[dims[1], dims[0]]).unwrap();
      tensor
    };
    let mut wq = vec![];
    let mut wk = vec![];
    ...
    for layer in 0..n_layers {
      wq.push(
        Self::load_tensor(gf, &format!("blk.{}.attn_q.weight", layer))
          .unwrap()
          .0,
      );
      wk.push(
        Self::load_tensor(gf, &format!("blk.{}.attn_k.weight", layer))
          .unwrap()
          .0,
      );
      wv.push(
        Self::load_tensor(gf, &format!("blk.{}.attn_v.weight", layer))
          .unwrap()
          .0,
      );
      wo.push(
        Self::load_tensor(gf, &format!("blk.{}.attn_output.weight", layer))
          .unwrap()
          .0,
      );
      ...
      rms_att_weight.push(
        Self::load_tensor(gf, &format!("blk.{}.attn_norm.weight", layer))
          .unwrap()
          .0,
      );
      rms_ffn_weight.push(
        Self::load_tensor(gf, &format!("blk.{}.ffn_norm.weight", layer))
          .unwrap()
          .0,
      );
    }
    let rms_final_weight =
      Self::load_tensor(gf, "output_norm.weight").unwrap().0;
    let wcls = {
      let (tensor, dims) = Self::load_tensor(gf, "output.weight").unwrap();
      let tensor = tensor.view(&[dims[1], dims[0]]).unwrap();
      tensor
    };
    Ok(Llama2Weights {
  ...
  pub(crate) fn load_tensor<'a>(
    ...
    let tensor = Tensor::new(f32_data, info.dimensions().to_vec()).unwrap();
  ...
  fn load_config(gf: &GGUFFile) -> Llama2Config {
    ...
    let n_heads =
      gf.metadata().get_u32("llama.attention.head_count").unwrap() as usize;
    let n_layers = gf.metadata().get_u32("llama.block_count").unwrap() as usize;
    let hidden_dim =
      gf.metadata().get_u32("llama.feed_forward_length").unwrap() as usize;
    let n_kv_heads = gf
      .metadata()
      .get_u32("llama.attention.head_count_kv")
      .unwrap() as usize;
    let seq_len =
      gf.metadata().get_u32("llama.context_length").unwrap() as usize;
    let vocab_size = gf
      .metadata()
      .get_string_array("tokenizer.ggml.tokens")
      .unwrap()
      .len();
    let embedding_dim =
      gf.metadata().get_u32("llama.embedding_length").unwrap() as usize;
    Llama2Config {
      n_heads,
      n_kv_heads,
      n_layers,
...
impl Llama2Loader for Llama2GgufLoader {
  fn load(&self) -> Result<(Llama2Config, Llama2Weights, Llama2Tokenizer)> {
    ...
    let weights = Self::load_weights(&gf, config.n_layers).unwrap();
    Ok((config, weights, tokenizer))*/
    Err(Llama2Error {
      kind: Llama2ErrorKind::IOError,
      message: "failed to load gguf file".to_string(),
      source: None,
    })
  }
}
...
pub struct Llama2CheckpointLoader {
  checkpoint_mmap: Vec<u8>,
  tokenizer_buffer: Vec<u8>,
}

impl Llama2Loader for Llama2CheckpointLoader {
  fn load(&self) -> Result<(Llama2Config, Llama2Weights, Llama2Tokenizer)> {
    let mut r = self.reader();
    let conf = Self::load_config(&mut r).unwrap();
    let weights = Self::load_weights(&mut r, &conf).unwrap();
    let mut tokenizer_loader =
      Llama2TokenizerLoader::new(self.tokenizer_buffer.clone()).unwrap();
    let tokenizer = tokenizer_loader.load(conf.vocab_size).unwrap();
    Ok((conf, weights, tokenizer))
  }
}

impl Llama2CheckpointLoader {
  pub fn new(model_buffer: Vec<u8>, tokenizer_buffer: Vec<u8>) -> Result<Self> {
    // prepare the tokenizer loader
    Ok(Self {
      checkpoint_mmap: model_buffer,
      tokenizer_buffer,
    })
  }
  ...
  fn load_config(r: &mut Llama2CheckpointReader<'_>) -> Result<Llama2Config> {
    let dim = r.read_i32().unwrap() as usize;
    let hidden_dim = r.read_i32().unwrap() as usize;
    let n_layers = r.read_i32().unwrap() as usize;
    let n_heads = r.read_i32().unwrap() as usize;
    let n_kv_heads = r.read_i32().unwrap() as usize;
    let vocab_size = r.read_i32().unwrap() as usize;
    let seq_len = r.read_i32().unwrap() as usize;
    Ok(Llama2Config {
      embedding_dim: dim,
      hidden_dim,
      n_layers,
  ...
  fn load_weights<'a>(
    ...
    weights.token_embedding_table = r
      .read_tensor(vec![conf.vocab_size, conf.embedding_dim])
      .unwrap();
    ...
    weights.w3 = r
      ...
      .subtensors()
      .unwrap();
    weights.rms_final_weight = r.read_tensor(vec![conf.embedding_dim]).unwrap();
    let _ = r.read_tensor(vec![conf.seq_len * head_size / 2]).unwrap(); // skip what used to be freq_cis_real (for RoPE)
    let _ = r.read_tensor(vec![conf.seq_len * head_size / 2]).unwrap(); // skip what used to be freq_cis_imag (for RoPE)
...
impl Llama2Tokenizer {
  ...
  pub fn decode_string(&self, tokens: &[usize]) -> Result<String> {
    ...
    for token in tokens {
      let piece = self.decode(prev_token, *token).unwrap();
...
impl Llama2TokenizerLoader {
  pub fn new(tokenizer_buffer: Vec<u8>) -> Result<Self> {
    let f = std::io::BufReader::new(std::io::Cursor::new(tokenizer_buffer));
    Ok(Self { r: Box::new(f) })
  }
  ...
  pub fn load(&mut self, vocab_size: usize) -> Result<Llama2Tokenizer> {
    ...
    let token_buf_len = self.read_i32().unwrap() as usize;
    for i in 0..vocab_size {
      vocab_scores[i] = self.read_f32().unwrap();
      let len = self.read_i32().unwrap();
      vocab[i] = self.read_string(len as usize).unwrap();
    }
  ...
  fn read_string(&mut self, len: usize) -> Result<String> {
    let buf = self.read_bytes(len).unwrap();
...
impl<'a> Llama2Runner<'a> {
  ...
  fn rope(&mut self, pos: usize) {
    for i in (0..self.conf.embedding_dim).step_by(2) {
      ...
      let freq =
        1.0 / 10000_f32.powf(head_dim as f32 / self.head_size() as f32);
      let val = pos as f32 * freq;
      let fcr = val.cos();
      let fci = val.sin();
      let rotn = if i < self.kv_dim() { 2 } else { 1 }; // how many vectors.unwrap() 2 = q & k, 1 = q only
      for v in 0..rotn {
        let vec = if v == 0 {
          &mut self.state.q
        } else {
  ...
  fn multi_head_attention(&mut self, l: usize, pos: usize) {
    ...
    self
      ...
      .for_each(|(h, (attn, xb))| {
        ...
        for t in 0..(pos + 1) {
          let k = &self.state.key_cache[l][t]
            [kvh * head_size..kvh * head_size + head_size];
        ...
        xb.fill(0.0);
        for t in 0..pos + 1 {
          let v = &self.state.value_cache[l][t]
            [kvh * head_size..kvh * head_size + head_size];
  ...
  fn ffn(&mut self, l: usize) -> Result<()> {
    ...
    for i in 0..hidden_dim {
      self.state.hb[i] =
        self.state.hb[i] * (1.0 / (1.0 + (-self.state.hb[i]).exp()));
    }
  ...
  pub fn forward(&mut self, token: usize, pos: usize) -> Result<&mut [f32]> {
    ...
    let content_row =
      self.weights.token_embedding_table.subtensor(token).unwrap();
...
impl<'a> Llama2RunnerOutputGenerator<'a> {
  fn new(
    ...
    let prompt_tokens = runner.tokenizer.encode(prompt, true, false).unwrap();
    if prompt_tokens.is_empty() {
      return Err(Llama2Error {
        kind: Llama2ErrorKind::BadInput,
        message: "something is wrong, expected at least 1 prompt token"
          .to_string(),
        source: None,
      });
    }
  ...
  pub fn average_tokens_per_seconds(&self) -> f32 {
    let total_time = self.total_time as f32;
  ...
  fn forward_next(&mut self) -> Result<Option<String>> {
    ...
    let logits = self.runner.forward(self.token, self.pos).unwrap();
    ...
    self.total_time +=perf.now() - start_time;
