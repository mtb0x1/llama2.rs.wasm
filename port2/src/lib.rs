...
impl TransformerWeights {
  fn from_buf_reader(
    f: &mut BufReader<std::io::Cursor<Vec<u8>>>,
    c: &Config,
  ) -> Self {
...
fn transformer(
  ...
  let freq_cis_real_row =
    &w.freq_cis_real[pos * (head_size / 2)..(pos + 1) * (head_size / 2)];
  let freq_cis_imag_row =
    &w.freq_cis_imag[pos * (head_size / 2)..(pos + 1) * (head_size / 2)];
  ...
  for l in 0..(p.n_layers as usize) {
    ...
    s.key_cache[(loff + pos * dim)..(loff + (pos + 1) * dim)]
      .copy_from_slice(&s.k);
    s.value_cache[(loff + pos * dim)..(loff + (pos + 1) * dim)]
      .copy_from_slice(&s.v);
...
) -> (Vec<(String, Score)>, u32) {
  let mut rdr = std::io::BufReader::new(std::io::Cursor::new(tokenizer_buffer));
  // BufReader::new(File::open("tokenizer.bin").expect("Couldn't load
  // tokenizer.bin"));
...
fn read_vec<T: FromBytes>(
  rdr: &mut BufReader<std::io::Cursor<Vec<u8>>>,
  size: i32,
) -> Vec<T> {
