...
impl Tokenizer {
  pub fn read(data: &[u8], vocab_size: usize) -> io::Result<Tokenizer> {
    ...
    for _ in 0..vocab_size {
      ...
      let len = reader
        .read_u32::<LittleEndian>()
        .map_err(|_| to_io_err("Failed to read token length"))?
        as usize;
    ...
    for (n, item) in vocab.iter().enumerate() {
      vocab_index.insert(item.clone(), n);
    }
