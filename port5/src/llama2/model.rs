...
use ndarray::prelude::*;
use ndarray::{RemoveAxis, StrideShape};
  ...
  D: Dimension,
{
  ...
  pub fn quantize(&self) -> Tensor<D> {
    match self {
      ...
      Tensor::F32(a) => {
        Tensor::Qi8(QintArray::quantize(DEFAULT_STRIDE, a.view()))
      },
    }
  }
}
    ...
    S: Into<StrideShape<D>>,
  {
    if self.conf.q_type == QuantizationType::None {
      ...
      Tensor::Qi8(QintArray {
        ...
        scaling: read_array(
          self.buf,
          scaling_dim(shape.raw_dim(), self.conf.q_stride),
        ),
  ...
  D: Dimension + RemoveAxis,
{
  ...
  pub fn unwrap_f32(&self) -> ArrayView<'_, f32, D> {
    match self {
...
impl Config {
  pub fn read(data: &[u8], logs: &mut Vec<String>) -> io::Result<Self> {
