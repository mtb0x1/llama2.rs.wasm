...
#[derive(Debug, Clone)]
pub struct QintArrayView<'a, T, D>
where
  D: Dimension,
{
  pub stride: usize,
...
// and stride.
pub fn scaling_dim<D>(dim: &D, stride: usize) -> D
where
  D: Dimension,
{
...
fn quant_i8<D>(stride: usize, fpa: ArrayView<f32, D>) -> QintArray<i8, D>
where
  D: Dimension,
{
  ...
  for bucket in fpa.as_slice().unwrap().chunks(stride) {
    let (min, max) =
      bucket.iter().fold((f32::MAX, f32::MIN), |(min, max), &x| {
        (min.min(x), max.max(x))
      });
  ...
  QintArray {
    stride,
    scaling: Array::from_shape_vec(
      scaling_dim(&fpa.raw_dim(), stride),
      scaling,
    )
    .unwrap(),
...
mod tests {
  ...
  fn it_quantizes() {
    ...
    assert_eq!(
      quant_i8(2, fpa.view()).arr.to_vec(),
      vec![-127, -49, -64, 127, 109, 127]
    );
  }
  ...
  fn it_has_little_loss() {
    ...
    let lo = loss(
      &dq_arr.view(),
      &Array::from_vec(vec![-0.9, -0.35, -0.1, 0.2, 0.6, 0.7]).view(),
    );
  ...
  fn it_quantizes_many_with_little_loss() {
    ...
    for n in 0..1000 {
      let fpa =
        Array::from_vec((0..2048).map(|_| 4.0 * random::<f32>()).collect());
