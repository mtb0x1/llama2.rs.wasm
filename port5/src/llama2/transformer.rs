...
fn matmul(mut xout: ArrayViewMut1<ATy>, x: &Array1<ATy>, w: &TensorView2<'_>) {
...
fn sin_cos(
  ...
  (0..dim / 2)
    .map(|i| {
      ...
      let pos_freq =
        (pos as f32) * (1.0 / theta.powf(head_dim / head_size as ATy));
      pos_freq.sin_cos()
    })
    .collect()
}
...
impl Transformer {
  pub fn new(config: &Config, weights: Weights) -> Transformer {
    ...
    Transformer {
      ...
      key_cache: Array3::from_elem(
        (config.n_layers, config.seq_len, config.dim),
        ATY_ZERO,
      ),
      value_cache: Array3::from_elem(
        (config.n_layers, config.seq_len, config.dim),
        ATY_ZERO,
      ),
      conf: config.clone(),
      ...
      rope_theta: if config.vocab_size > 32000 {
        1000000.0
      } else {
        10000.0
      },
  ...
  pub fn forward(&mut self, token: usize, pos: usize) -> ArrayViewMut1<ATy> {
    let sin_cos =
      sin_cos(pos, self.conf.dim, self.conf.head_size(), self.rope_theta);
    // copy the token embedding into x
    self
      .x
      .assign(&self.w.tet.index_axis(Axis(0), token).to_f32().view());
    ...
    for l in 0..self.conf.n_layers {
      ...
      rmsnorm(
        self.xb.view_mut(),
        self.x.view(),
        self.w.rms_att.index_axis(Axis(0), l),
      );
      ...
      matmul(
        self.q.view_mut(),
        &self.xb,
        &self.w.wq.index_axis(Axis(0), l),
      );
      matmul(
        self.k.view_mut(),
        &self.xb,
        &self.w.wk.index_axis(Axis(0), l),
      );
      matmul(
        self.v.view_mut(),
        &self.xb,
        &self.w.wv.index_axis(Axis(0), l),
      );
      ...
      rmsnorm(
        self.xb.view_mut(),
        self.x.view(),
        self.w.rms_ffn.index_axis(Axis(0), l),
      );
      ...
      matmul(
        self.hb.view_mut(),
        &self.xb,
        &self.w.w1.index_axis(Axis(0), l),
      );
      matmul(
        self.hb2.view_mut(),
        &self.xb,
        &self.w.w3.index_axis(Axis(0), l),
      );
  ...
  fn rope_enc(&mut self, sin_cos: &[(ATy, ATy)]) {
