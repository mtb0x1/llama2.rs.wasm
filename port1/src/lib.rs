...
impl Config {
  /// Read raw bytes and force those to be our config type (which conforms to C
  /// mem layout)
  fn from_file(data: &mut Vec<u8>) -> Self {
    let mut model_bin = std::io::BufReader::new(std::io::Cursor::new(data));
    // File::open(path).expect(format!("Couldn't find model file at {}",
    // path).as_str());
    let mut buffer = [0; CONF_SIZE];
    model_bin.read_exact(&mut buffer).unwrap();

    let raw_conf =
      unsafe { mem::transmute::<[u8; CONF_SIZE], [i32; CONF_VALS]>(buffer) };
...
impl Vocab {
  fn from_file(vocab_size: usize, data: &mut Vec<u8>) -> Self {
    ...
    // File::open(path).expect(format!("Couldn't find tokenizer file at {}",
    // path).as_str());
    let mut len = [0; 4];
    let mut val = [0; 1];
...
fn _att_head_impl(
  ...
  // cahced vals have head_size values in it. we need to go over all t vals and
  // update xb dst is head_size part of xb, we gonna add t (actually pos values)
  // values into it
  let dst = xb;
  // clear dst
  dst.iter_mut().for_each(|v| *v = 0f32);
  // this is different from Karphaty's impl. first go over all head size values,
  // than skip time stamp Why Karphaty's inner loop does C.dim jumps?
...
impl RunState {
  ...
  fn attention(&mut self, pos: usize, layer: usize, cfg: &Config) {
    ...
    let attn_lambda = |h| {
      ...
      let layer_cached_vals = _uncheked_slice(
        &self.value_cache,
        0,
        cfg.n_layers * cfg.seq_len * cfg.dim,
      );
  ...
  fn step(
    ...
    // last rmsnorm
    let ss =
      self.x.iter().fold(0f32, |init, &v| init + v * v) / (self.x.len() as Ty);
    let ss = (1 as Ty) / (ss + 1e-5).sqrt();
    self
      .x
      .iter_mut()
