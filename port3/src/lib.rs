...
impl Config {
  fn from_file(weights: &Vec<u8>) -> Result<Self> {
    ...
    let raw_config = unsafe {
      std::mem::transmute::<
        [u8; std::mem::size_of::<RawConfigI32>()],
        RawConfigI32,
      >(weights[..config_byte_size].try_into().unwrap())
    };
    ...
    Ok(Self {
      ...
      vocab_size: raw_config[5].unsigned_abs() as usize,
...
impl Tokenizer {
  fn from_file(tokenizer: Vec<u8>, vocab_size: usize) -> Result<Tokenizer> {
    let mut vocab = Tokenizer::default();
    vocab.vocab_scores.reserve(vocab_size);
    vocab.vocab.reserve(vocab_size);

    let mut reader = BufReader::new(std::io::Cursor::new(tokenizer));

    // Read max_token_length
    let max_token_length_buffer =
      read_n(&mut reader, std::mem::size_of::<u32>())?;
    vocab.max_token_length =
      byteorder::LittleEndian::read_u32(&max_token_length_buffer) as usize;
    ...
    for _ in 0..vocab_size {
      ...
      let string =
        String::from_utf8(string_buffer).expect("could not read word");
  ...
  fn lookup_word(&self, word: &str) -> Option<usize> {
    self.word_to_token_id.get(word).copied()
  }
...
impl TransformerWeights {
  fn from_file(weights: &[u8], config: &Config) -> Result<Self> {
    ...
    let token_embedding_table: Vec<f32> =
      byte_chunk_to_vec(&weights[offset..], token_embedding_table_size);
    ...
    let rms_att_weight: Vec<f32> =
      byte_chunk_to_vec(&weights[offset..], rms_att_weight_size);
    ...
    let rms_final_weights: Vec<f32> =
      byte_chunk_to_vec(&weights[offset..], rms_final_weights_size);
    ...
    let freq_cis_real: Vec<f32> =
      byte_chunk_to_vec(&weights[offset..], freq_cis_real_size);
    offset += freq_cis_real_size * std::mem::size_of::<f32>();

    let freq_cis_imag_size = config.seq_len * config.head_size / 2;
    let freq_cis_imag: Vec<f32> =
      byte_chunk_to_vec(&weights[offset..], freq_cis_imag_size);
    ...
    let wcls: Vec<f32> = if config.shared_weights {
      ...
      byte_chunk_to_vec(&weights[offset..], wcls_size)
    };
  ...
  fn memory_usage_in_bytes(&self) -> usize {
    (self.num_parameters() + self.token_embedding_table.len())
      * std::mem::size_of::<f32>()
  }
}
...
fn softmax(logits: &mut [f32]) {
  ...
  for (_, logit) in logits.iter().enumerate().take(n).skip(1) {
    max_logit = max_logit.max(*logit);
  }

  // Exponentiate and sum logits
  let mut sum = 0.0;
  for (_, logit) in logits.iter_mut().enumerate().take(n) {
...
impl<'a> LLaMA2<'a> {
  ...
  fn attn_rope(&mut self, _layer: usize, pos: usize) {
  ...
  fn multihead_attn(&mut self, layer: usize, pos: usize) {
    ...
    self
      ...
      .for_each(|(h, (attn_scores, xb))| {
        ...
        for (t, item) in attn_scores.iter_mut().enumerate().take(pos + 1) {
          let timestep_and_layer_offset =
            layer_offset_for_cache + t * self.config.dim; // key_cache[l, t]
                                                          // for the current key, we need to select the correct range which corresponds to the current head
          let key_vector_from =
            timestep_and_layer_offset + h * self.config.head_size;
          let key_vector_to =
            timestep_and_layer_offset + (h + 1) * self.config.head_size;
          let key_vector = &self.key_cache[key_vector_from..key_vector_to];

          *item = inner_product(q, key_vector) / sqrt_d;
        }
        ...
        for (t, item) in attn_scores.iter().enumerate().take(pos + 1) {
          let timestep_and_layer_offset =
            layer_offset_for_cache + t * self.config.dim; // value_cache[l, t]
                                                          // for the current value, we need to select the correct range which corresponds to the current head
          let value_vector_from =
            timestep_and_layer_offset + h * self.config.head_size;
          let value_vector_to =
            timestep_and_layer_offset + (h + 1) * self.config.head_size;
          let value_vector =
            &self.value_cache[value_vector_from..value_vector_to];

          // weighted sum with attention scores as weights
          let attention_weight = *item;
  ...
  fn generate(
    ...
    for pos in prompt_tokens.len()..(n_tokens - 1) {
      ...
      if temperature == 0.0 {
        ...
        self.logits.iter_mut().for_each(|p| *p /= temperature);
        softmax(self.logits.as_mut_slice());
...
pub fn port3_main_wasm(
  ...
  logs.push("using port3".into());

  logs.push("Loading config...".into());
  let config = Config::from_file(&model_buffer).unwrap();
  logs.push("Loaded config".into());

  logs.push("Loading vocab...".into());
  let tokenizer =
    Tokenizer::from_file(tokenizer_buffer, config.vocab_size).unwrap();

  logs.push("Loading weights...".into());
  let transformer_weights =
    TransformerWeights::from_file(&model_buffer, &config).unwrap();
  ...
  let llama_memory_mib =
    llama2.memory_usage_in_bytes() as f32 / ((1usize) << 20) as f32;

  let alog = format!("Memory usage in MiB: {llama_memory_mib}");
  logs.push(alog);
  let prompt_tokens = tokenizer.bpe_encode(&prompt);
  let generated_tokens = llama2.generate(&prompt_tokens, steps, temperature);

  let time_elapsed = perf.now() - start;
  let tokens_per_sec = (steps as f64) / time_elapsed;
  let alog = format!("tokens / seconds = {:.2?}", tokens_per_sec);
  logs.push(alog);

  let alog = format!("{}", prompt);
  logs.push(alog);
  for token in generated_tokens {
    if token == 1 && tokenizer.decode(token).starts_with(' ') {
      let alog = format!("{}", &tokenizer.decode(token)[1..]);
      logs.push(alog);
      result.push(tokenizer.decode(token)[1..].into())
    } else {
      let alog = format!("{}", tokenizer.decode(token));
      logs.push(alog);
      result.push(tokenizer.decode(token).into())
    };
  }
  ...
  let token_per_second = JsValue::from(tokens_per_sec * 1000f64);
